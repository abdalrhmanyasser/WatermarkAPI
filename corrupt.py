...
def corrupt_audio(
    ...
    simulate_platform="none"  # NEW: "youtube", "instagram", "whatsapp", or "none"
):
    print("\n--- CORRUPTING ---")
    ...
    # ... [Keep your Volume, Noise, LPF, MP3, Cut, and Speed blocks here exactly as before] ...
    # (For brevity, assuming the standard attacks from the previous script are here)
    ...
    if simulate_platform != "none":
        ...
        # We must save the current state to a temporary WAV so FFmpeg can read it
        temp_in = "temp_platform_in.wav"
        torchaudio.save(temp_in, wav, sr)
        
        if simulate_platform == "youtube":
            # YouTube standard: 128kbps AAC
            ...
            # WhatsApp Voice: 24kbps Opus, forced Mono
        ...
        # Run FFmpeg and catch errors if it fails
        try:
            subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
        except subprocess.CalledProcessError as e:
            print(f"[!] FFmpeg failed: {e.stderr.decode('utf-8')}")
            # Clean up input before returning
            if os.path.exists(temp_in): os.remove(temp_in)
            return False
        
        # Load the simulated platform audio back into PyTorch
